...
async def test_organization_and_user_round_trip(uow_factory) -> None:
...
async def test_policy_round_trip_preserves_value_objects(uow_factory) -> None:
    async with (uow := uow_factory()):
        await uow.organizations.add(make_org())
...
async def test_risk_update_round_trip(uow_factory) -> None:
    async with (uow := uow_factory()):
        await uow.organizations.add(make_org())
...
async def test_framework_composite_key_round_trip(uow_factory) -> None:
    async with (uow := uow_factory()):
...
async def test_mission_round_trip_with_children(uow_factory) -> None:
    async with (uow := uow_factory()):
        await uow.organizations.add(make_org())
