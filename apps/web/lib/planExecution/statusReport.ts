 ...
 *
 * An export OF THE GOVERNANCE PROGRAM (CLAUDE.md §3 pillar 10). It is not a replacement for the
 * Home Dashboard's own export, which is a separate artifact of a separate surface and is left
 * exactly as it is; the two answer different questions and neither derives from the other.
 *
...
import { MATURITY_DIMENSION_ORDER } from "./types";
import { PRIORITY_RANK } from "./grouping";
...
function stampHeader(
  ...
  if (isContinuationPage) {
    page.drawText("Governance Status Report", {
      x: MARGIN,
      y,
      size: 9,
      font: bold,
